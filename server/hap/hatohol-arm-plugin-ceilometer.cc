...
protected:
	...
	HatoholError parseAlarmElement(JSONParserAgent &parser,
	                               VariableItemTablePtr &tablePtr,
                                       const unsigned int &index);
	TriggerStatusType parseAlarmState(const string &state);
	SmartTime parseStateTimestamp(const string &stateTimestamp);
	uint64_t generateHashU64(const string &str);

	bool startObject(JSONParserAgent &parser, const string &name);
	bool read(JSONParserAgent &parser, const string &member, string &dest);
	bool parserEndpoints(JSONParserAgent &parser,
                             const unsigned int &index);
...
bool HapProcessCeilometer::parserEndpoints(JSONParserAgent &parser,
                                    const unsigned int &index)
{
	...
	JSONParserRewinder parserRewinder(parser);
	parserRewinder.pushElement();
	...
	if (!startObject(parser, "endpoints"))
		return false;
	parserRewinder.pushObject();
...
HatoholError HapProcessCeilometer::parseAlarmElement(
  JSONParserAgent &parser, VariableItemTablePtr &tablePtr,
  const unsigned int &index)
{
	if (!parser.startElement(index)) {
		MLPL_ERR("Failed to parse an element, index: %u\n", index);
	...
	if (!startObject(parser, "threshold_rule"))
		return HTERR_FAILED_TO_PARSE_JSON_DATA;
	parserRewinder.pushObject();
...
bool HapProcessCeilometer::startObject(
  JSONParserAgent &parser, const string &name)
{
...
bool HapProcessCeilometer::read(
  JSONParserAgent &parser, const string &member, string &dest)
{
